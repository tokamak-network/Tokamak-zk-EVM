...
use libs::vector_operations::{matrix_matrix_mul, transpose_inplace};
...
fn matrix_matrix_mul_original(
    ...
    ScalarCfg::mul(
        HostSlice::from_slice(&ext_lhs_mat),
        HostSlice::from_slice(&ext_rhs_mat),
        mul_res_buff,
        &vec_ops_cfg,
    )
    .unwrap();

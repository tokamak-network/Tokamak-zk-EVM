...
mod safe_cpu_fallback_tests {
    ...
    use icicle_core::vec_ops::VecOps; // <-- Add this line to bring the trait into scope

    #[test]
    fn test_ntt_transpose_equivalence() {
        // 예제: 3x2 행렬
        let x_size = 2; // 행
        let y_size = 2; // 열
        let size = x_size * y_size;
        ...
        // 1. 랜덤 데이터 생성
        let random_coeffs: Vec<ScalarField> = ScalarCfg::generate_random(size).to_vec();

        // [1] (A) 방법: columns_batch = true (원본 그대로)
        let mut input_dev = DeviceVec::<ScalarField>::device_malloc(size).unwrap();
        input_dev.copy_from_host(HostSlice::from_slice(&random_coeffs)).unwrap();
        let mut out_a = DeviceVec::<ScalarField>::device_malloc(size).unwrap();
        let mut cfg_a = ntt::NTTConfig::<ScalarField>::default();
        cfg_a.batch_size = y_size as i32;
        cfg_a.columns_batch = true;
        ...
        ntt::ntt(&input_dev, ntt::NTTDir::kForward, &cfg_a, &mut out_a).unwrap();

        ntt::release_domain::<ScalarField>().unwrap();

        // 결과 host로 복사
        let mut result_a = vec![ScalarField::zero(); size];
        out_a.copy_to_host(HostSlice::from_mut_slice(&mut result_a)).unwrap();

        // [2] (B) 방법: 2번 트랜스포즈 + columns_batch = false (행 우선)
        ...
        // 원본을 먼저 트랜스포즈 (행렬 [행, 열] → [열, 행])
        input_tr.copy_from_host(HostSlice::from_slice(&random_coeffs)).unwrap();
        ScalarCfg::transpose(
            &input_tr,
            x_size as u32,
            y_size as u32,
        ...
        let mut cfg_b = ntt::NTTConfig::<ScalarField>::default();
        cfg_b.batch_size = y_size as i32;
        cfg_b.columns_batch = false;
        ...
        ntt::ntt(&output_tr, ntt::NTTDir::kForward, &cfg_b, &mut out_b).unwrap();

        // 다시 트랜스포즈해서 원래 shape로 맞춘다
        let mut out_b_final = DeviceVec::<ScalarField>::device_malloc(size).unwrap();
        ScalarCfg::transpose(
            &out_b,
            y_size as u32,
            x_size as u32,
        ...
        let mut result_b = vec![ScalarField::zero(); size];
        out_b_final.copy_to_host(HostSlice::from_mut_slice(&mut result_b)).unwrap();

        // 출력 비교
        println!("NTT 결과: columns_batch=true\n{:?}", result_a);
        println!("NTT 결과: transpose + columns_batch=false + 역transpose\n{:?}", result_b);

        assert_eq!(
            result_a, result_b,
            "NTT(columns_batch=true)와 NTT(transpose→columns_batch=false→역transpose) 결과가 다릅니다!"
        );
    }
}

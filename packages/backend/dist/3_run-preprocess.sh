...
if [ ! -x "$LOCAL" ]; then
    echo "[ERROR] Executable not found or not executable: $LOCAL"
    exit 1
fi
...
echo "[INFO] Launching: $LOCAL"
echo "[INFO] Using library path: ${DYLD_LIBRARY_PATH:-$LD_LIBRARY_PATH}"

exec "$LOCAL" "$SCRIPT_DIR/resource/qap_compiler/library" "$SCRIPT_DIR/resource/synthesizer/outputs" "$SCRIPT_DIR/resource/setup/output" "$SCRIPT_DIR/resource/preprocess/output"

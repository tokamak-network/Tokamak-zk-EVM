...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let setup_params = SetupParams::from_path(setup_file_name)?;
    ...
    let subcircuit_infos = SubcircuitInfo::from_path(subcircuit_file_name)?;

    // Load global wire list
    let global_wire_file_name = "globalWireList.json";
    let global_wire_list = read_global_wire_list_as_boxed_boxed_numbers(global_wire_file_name)?;
...

    {
        ...
        for i in 0..s_d {
            ...
            let compact_r1cs = SubcircuitR1CS::from_path(&r1cs_path, &setup_params, &subcircuit_infos[i])?;
    ...
    println!("Writing the sigma into Binary...");
    output_path = "setup/trusted-setup/output/combined_sigma.bin";
    let file = File::create(output_path)?;
    let writer = BufWriter::new(file);
    bincode::serialize_into(writer, &sigma)?;

    println!("Writing the sigma into JSON...");
    output_path = "setup/trusted-setup/output/sigma_verify.json";
    sigma.write_into_json_for_verify(output_path)?;
    output_path = "setup/trusted-setup/output/sigma_preprocess.json";
    sigma.write_into_json_for_preprocess(output_path)?;
    let lap = start.elapsed();

...
use mpc_setup::mpc_utils::{
    poly_mult_using_outer_product, thread_safe_compute_langrange_i_coeffs, thread_safe_compute_langrange_i_poly,
};
    ...
    for i in 0..s_max {
        for j in 0..m_i {
            ...
            if !qap.u_j_X[j + l].is_zero() {
                ...
                poly_mult_using_outer_product(&qap.u_j_X[j + l], &li_y_vec[i], &mut multpxy_coeffs);
            ...
            if !qap.v_j_X[j + l].is_zero() {
                ...
                poly_mult_using_outer_product(&qap.v_j_X[j + l], &li_y_vec[i], &mut multpxy_coeffs);
            ...
            if !qap.w_j_X[j + l].is_zero() {
                ...
                poly_mult_using_outer_product(&qap.w_j_X[j + l], &li_y_vec[i], &mut multpxy_coeffs);
            ...
            poly_mult_using_outer_product(&kj_x_vec[j], &li_y_vec[i], &mut xy_coeffs);
    ...
    for i in 0..s_max {
        for j in (l + m_i)..m_d {
            ...
            if !qap.u_j_X[j].is_zero() {
                poly_mult_using_outer_product(&qap.u_j_X[j], &li_y_vec[i], &mut multpxy_coeffs);
                //out = out + sum_vector_dot_product(&multpxy_coeffs, &alpha1xyG1s);
                coeffs.extend(&multpxy_coeffs);
                commits.extend(&alpha1xyG1s);
            }
            ...
            if !qap.v_j_X[j].is_zero() {
                poly_mult_using_outer_product(&qap.v_j_X[j], &li_y_vec[i], &mut multpxy_coeffs);
                //out = out + sum_vector_dot_product(&multpxy_coeffs, &alpha2xyG1s);
                coeffs.extend(&multpxy_coeffs);
                commits.extend(&alpha2xyG1s);
            }
            ...
            if !qap.w_j_X[j].is_zero() {
                poly_mult_using_outer_product(&qap.w_j_X[j], &li_y_vec[i], &mut multpxy_coeffs);
                //out = out + sum_vector_dot_product(&multpxy_coeffs, &alpha3xyG1s);
                coeffs.extend(&multpxy_coeffs);
                commits.extend(&alpha3xyG1s);
            }
...
fn compute_gamma_part_i(
    ...
    for j in start..end {
        ...
        //lookup with alpha x^i y^j
        if !qap.u_j_X[j].is_zero() {
            poly_mult_using_outer_product(&qap.u_j_X[j], &lag_i, &mut multpxy_coeffs);
            coeffs.extend(&multpxy_coeffs);
            commits.extend(alpha1xyG1s);
        }
        //lookup with alpha^2 x^i y^j
        if !qap.v_j_X[j].is_zero() {
            poly_mult_using_outer_product(&qap.v_j_X[j], &lag_i, &mut multpxy_coeffs);
            coeffs.extend(&multpxy_coeffs);
            commits.extend(alpha2xyG1s);
        }
        //lookup with alpha^3 x^i y^j
        if !qap.w_j_X[j].is_zero() {
            poly_mult_using_outer_product(&qap.w_j_X[j], &lag_i, &mut multpxy_coeffs);
            coeffs.extend(&multpxy_coeffs);
            commits.extend(alpha3xyG1s);
        }
        if coeffs.len() > 0 {

/// Benchmark MSM with 75% sparse data (75% zeros)
mod common;
...
fn bench_sparse75(c: &mut Criterion) {
    check_device();

    println!("\n========================================");
    println!("MSM Benchmark: Sparse 75% zeros");
    println!("MSM size: {} x {} batches", MSM_SIZE, BATCH_CNT);
    println!("========================================\n");

    let mut group = c.benchmark_group("msm_sparse75");
    ...
    println!("Generating sparse75 data...");
    let scalars = generate_sparse_data(BATCH_CNT, MSM_SIZE, 0.75);
    print_zero_stats("Sparse75", &scalars);
...
criterion_group!(benches, bench_sparse75);

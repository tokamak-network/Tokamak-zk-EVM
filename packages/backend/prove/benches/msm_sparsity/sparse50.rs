/// Benchmark MSM with 50% sparse data (50% zeros)
mod common;
...
fn bench_sparse50(c: &mut Criterion) {
    check_device();

    println!("\n========================================");
    println!("MSM Benchmark: Sparse 50% zeros");
    println!("MSM size: {} x {} batches", MSM_SIZE, BATCH_CNT);
    println!("========================================\n");

    let mut group = c.benchmark_group("msm_sparse50");
    ...
    println!("Generating sparse50 data...");
    let scalars = generate_sparse_data(BATCH_CNT, MSM_SIZE, 0.50);
    print_zero_stats("Sparse50", &scalars);
...
criterion_group!(benches, bench_sparse50);

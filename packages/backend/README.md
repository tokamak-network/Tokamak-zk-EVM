...
For every build profile, Cargo build scripts:

- build the local `packages/frontend/qap-compiler` subcircuit library
- read the local `@tokamak-zk-evm/subcircuit-library` package version
- embed the generated `subcircuits/library` files into the backend binary
- do not fetch `@tokamak-zk-evm/subcircuit-library` from npm
- do not expose `--subcircuit-library` at runtime

Builds also emit metadata under `target/<profile>/`:
...
Each metadata file records the local bundled subcircuit-library version used for that build.
...
Launch targets mirror the embedded-subcircuit-library contract and do not pass
`--subcircuit-library` in either release or non-release profiles.

...
- Added Linux and macOS `--install --docker` support that installs through an Ubuntu 22 Docker image, records host-specific Docker bootstrap files under `~/.tokamak-zk-evm/<platform>/docker`, and runs backend preprocess, prove, and verify commands through that bootstrap when Docker is available.

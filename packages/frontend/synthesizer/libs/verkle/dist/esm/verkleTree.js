    ...
    async put(stem, suffixes, values = []) {
        ...
        if (foundPath.node !== null) {
            ...
            if (!isLeafVerkleNode(foundPath.node)) {
                throw new Error(`expected leaf node found at ${bytesToHex(stem)}. Got internal node instead`);
            }
            leafNode = foundPath.node;
            // Sanity check to verify we have the right leaf node
            if (!equalsBytes(leafNode.stem, stem)) {
                throw new Error(`invalid leaf node found. Expected stem: ${bytesToHex(stem)}; got ${bytesToHex(foundPath.node.stem)}`);
            }
        }
        else {

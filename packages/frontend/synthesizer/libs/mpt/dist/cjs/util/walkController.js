...
class WalkController {
    ...
    allChildren(node, key = []) {
        if (node instanceof index_js_1.LeafMPTNode) {
            return;
        }
        let children;
        if (node instanceof index_js_1.ExtensionMPTNode) {
            children = [[node.key(), node.value()]];
        }
        else if (node instanceof index_js_1.BranchMPTNode) {
    ...
    onlyBranchIndex(node, key = [], childIndex, priority) {
        if (!(node instanceof index_js_1.BranchMPTNode)) {

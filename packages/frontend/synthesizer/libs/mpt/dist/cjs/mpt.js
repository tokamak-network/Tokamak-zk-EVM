...
class MerklePatriciaTrie {
    ...
    async findPath(key, throwIfMissing = false, partialPath = {
        ...
        const onFound = async (_, node, keyProgress, walkController) => {
            ...
            else if (node instanceof index_js_2.LeafMPTNode) {
            ...
            else if (node instanceof index_js_2.ExtensionMPTNode) {
                this.DEBUG &&
    ...
    async walkAllValueNodes(onFound) {
        for await (const { node, currentKey } of this.walkTrieIterable(this.root(), [], undefined, async (node) => {
            return (node instanceof index_js_2.LeafMPTNode || (node instanceof index_js_2.BranchMPTNode && node.value() !== null));
        })) {
            await onFound(node, currentKey);
        }
    }
    ...
    async _createInitialNode(key, value) {
        const newNode = new index_js_2.LeafMPTNode((0, nibbles_js_1.bytesToNibbles)(key), value);
    ...
    async lookupNode(node) {
        if ((0, index_js_2.isRawMPTNode)(node)) {
            const decoded = (0, index_js_2.decodeRawMPTNode)(node);
        ...
        const decoded = (0, index_js_2.decodeMPTNode)(value);
    ...
    async _updateNode(k, value, keyRemainder, stack) {
        ...
        let matchLeaf = false;
        if (lastNode instanceof index_js_2.LeafMPTNode) {
            let l = 0;
            for (let i = 0; i < stack.length; i++) {
                const n = stack[i];
                if (n instanceof index_js_2.BranchMPTNode) {
                    l++;
                }
                else {
                    l += n.key().length;
            ...
            if (keyRemainder.length === 0 &&
                (0, nibbles_js_1.matchingNibbleLength)(lastNode.key(), key.slice(l)) === lastNode.key().length) {
                matchLeaf = true;
            }
        }
        if (matchLeaf) {
        ...
        else if (lastNode instanceof index_js_2.BranchMPTNode) {
            stack.push(lastNode);
            if (keyRemainder.length !== 0) {
                // add an extension to a branch node
                keyRemainder.shift();
                // create a new leaf
                const newLeaf = new index_js_2.LeafMPTNode(keyRemainder, value);
                stack.push(newLeaf);
            }
            else {
                lastNode.value(value);
            ...
            const newBranchMPTNode = new index_js_2.BranchMPTNode();
            // create a new extension node
            if (matchingLength !== 0) {
                const newKey = lastNode.key().slice(0, matchingLength);
                const newExtNode = new index_js_2.ExtensionMPTNode(newKey, value);
            ...
            if (lastKey.length !== 0) {
                ...
                if (lastKey.length !== 0 || lastNode instanceof index_js_2.LeafMPTNode) {
            ...
            if (keyRemainder.length !== 0) {
                ...
                const newLeafMPTNode = new index_js_2.LeafMPTNode(keyRemainder, value);
    ...
    async _deleteNode(k, stack) {
        const processBranchMPTNode = (key, branchKey, branchNode, parentNode, stack) => {
            ...
            if (parentNode === null || parentNode === undefined || parentNode instanceof index_js_2.BranchMPTNode) {
                // branch->?
                if (parentNode !== null && parentNode !== undefined) {
                    stack.push(parentNode);
                }
                if (branchNode instanceof index_js_2.BranchMPTNode) {
                    ...
                    const extensionNode = new index_js_2.ExtensionMPTNode([branchKey], new Uint8Array());
        ...
        if (branchNodes.length === 1) {
            ...
            if (this._opts.useNodePruning) {
                ...
                opStack.push({
                    type: 'del',
                    key: (0, index_js_2.isRawMPTNode)(branchNode) ? this.appliedKey(rlp_1.RLP.encode(branchNode)) : branchNode,
                });
            }
            // look up node
    ...
    async saveStack(key, stack, opStack) {
        ...
        while (stack.length) {
            ...
            if (node instanceof index_js_2.LeafMPTNode || node instanceof index_js_2.ExtensionMPTNode) {
                key.splice(key.length - node.key().length);
            }
            if (node instanceof index_js_2.ExtensionMPTNode && lastRoot !== undefined) {
                node.value(lastRoot);
            }
            if (node instanceof index_js_2.BranchMPTNode && lastRoot !== undefined) {
                ...
                await this.walkTrie(this.root(), async function (_, node, key, controller) {
                    ...
                    if (node instanceof index_js_2.BranchMPTNode) {
                        for (const item of node._branches) {
                            ...
                            if (item !== null &&
                                (0, util_1.bytesToUnprefixedHex)((0, index_js_2.isRawMPTNode)(item) ? controller.trie.appliedKey(rlp_1.RLP.encode(item)) : item) === dbkey) {
                                found = true;
                                return;
                            }
                        }
                    ...
                    if (node instanceof index_js_2.ExtensionMPTNode) {
    ...
    async _findDbNodes(onFound) {
        const outerOnFound = async (nodeRef, node, key, walkController) => {
            if ((0, index_js_2.isRawMPTNode)(nodeRef)) {
                if (node !== null) {
    ...
    async getValueMap(startKey = util_1.BIGINT_0, limit) {
        ...
        await this.walkAllValueNodes(async (node, currentKey) => {
            if (node instanceof index_js_2.LeafMPTNode) {
                const keyBytes = (0, nibbles_js_1.nibblesTypeToPackedBytes)(currentKey.concat(node.key()));
                if (!inRange) {

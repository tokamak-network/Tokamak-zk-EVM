...
async function unset(trie, parent, child, key, pos, removeLeft, stack) {
    if (child instanceof index_js_2.BranchMPTNode) {
        /**
    ...
    else if (child instanceof index_js_2.ExtensionMPTNode || child instanceof index_js_2.LeafMPTNode) {
        /**
        ...
        if (child instanceof index_js_2.LeafMPTNode) {
            ...
            if (_child instanceof index_js_2.LeafMPTNode) {
...
async function unsetInternal(trie, left, right) {
    ...
    while (true) {
        if (node instanceof index_js_2.ExtensionMPTNode || node instanceof index_js_2.LeafMPTNode) {
            // record this node on the stack
            stack.push(node);
        ...
        else if (node instanceof index_js_2.BranchMPTNode) {
            // record this node on the stack
            stack.push(node);
...
async function verifyMPTWithMerkleProof(rootHash, key, proof, useKeyHashingFunction) {
    const proofTrie = await (0, index_js_1.createMPTFromProof)(proof, {
        root: rootHash,
...
async function verifyRangeProof(rootHash, firstKey, lastKey, keys, values, proof, useKeyHashingFunction) {
    ...
    if (proof === null && firstKey === null && lastKey === null) {
        const trie = new mpt_js_1.MerklePatriciaTrie({ useKeyHashingFunction });
        for (let i = 0; i < keys.length; i++) {
            await trie.put((0, nibbles_js_1.nibblesTypeToPackedBytes)(keys[i]), values[i]);
        }
    ...
    if (proof !== null && firstKey !== null && lastKey === null) {
        // Zero element proof
        if (keys.length === 0) {
            const { trie, value } = await verifyMPTWithMerkleProof(rootHash, (0, nibbles_js_1.nibblesTypeToPackedBytes)(firstKey), proof, useKeyHashingFunction);
    ...
    const trie = await (0, index_js_1.createMPTFromProof)(proof, {
        useKeyHashingFunction,
        root: rootHash,
    });

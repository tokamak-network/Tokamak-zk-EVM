...
const exampleEntry = path.resolve(packageRoot, 'examples', 'privateStateRedeem', 'main.ts');
...
const buildGroupKey = (config: PrivateStateRedeemConfig) => {
...
const comparePermutations = async (configs: string[]) => {
  ...
  for (const configFile of configs) {
    ...
    const config = JSON.parse(await fs.readFile(configPath, 'utf8')) as PrivateStateRedeemConfig;
    ...
    if (!existing.contents.equals(permutationContents)) {
      throw new Error(
        [
          'Permutation mismatch for private-state redeem group:',
          `  key=${groupKey}`,
...
const main = async () => {
  ...
  try {
    console.log('[private-state-redeem-main] Preparing fresh anvil configs');
    ...
    if (configs.length === 0) {
      throw new Error(`No private-state redeem config files found in ${configDir}`);
    }
    ...
    for (const configFile of configs) {
      ...
      console.log(`[private-state-redeem-main] Running ${configFile}`);
      try {
        ...
        console.error(`[private-state-redeem-main] Failed for config: ${configPath}`);
        throw error;
      }
      await copyOutputs(destination);
    }

    console.log('[private-state-redeem-main] Validating permutation.json consistency');
    await comparePermutations(configs);
    console.log('[private-state-redeem-main] Validating instance_description.json contents');

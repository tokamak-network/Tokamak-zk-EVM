...
import { synthesizerArith, synthesizerEnvInf, synthesizerBlkInf, prepareEXTCodePt } from "../../tokamak/core/synthesizer.js";
...
export const synthesizerHandlers = new Map([
    ...
    [0x37, async function (runState) {
            ...
            if (dataLength.value !== BIGINT_0) {
                ...
                if (calldataMemoryPts.length > 0) {
                    memoryPtsToCopy = copyMemoryRegion(runState, dataOffset.value, dataLength.value, calldataMemoryPts, memOffset.value);
                }
                else {
                    ...
                    const entryToCopy = {
                        ...
                        dataPt: runState.synthesizer.loadEnvInf(runState.env.address.toString(), 'Calldata', bytesToBigInt(data), Number(dataOffset.value), Number(dataLength.value))
                    };
    ...
    [0x3c, async function (runState) {
            ...
            if (dataLengthPt.value !== BIGINT_0) {
                const dataPt = await prepareEXTCodePt(runState, addressPt.value, codeOffsetPt.value, dataLengthPt.value);
    ...
    [0x3e, async function (runState) {
            ...
            if (dataLength.value !== BIGINT_0) {
                const copiedMemoryPts = copyMemoryRegion(runState, returnDataOffset.value, dataLength.value, runState.returnMemoryPts, memOffset.value);
    ...
    [0x5e, async function (runState) {
            ...
            const copiedMemoryPts = copyMemoryRegion(runState, src.value, length.value, undefined, dst.value);
            for (const entry of copiedMemoryPts) {
                runState.memoryPt.write(entry.memOffset, entry.containerSize, entry.dataPt);
            }
    ...
    [0xf3, async function (runState) {
            ...
            const returnMemoryPts = copyMemoryRegion(runState, offset.value, length.value);

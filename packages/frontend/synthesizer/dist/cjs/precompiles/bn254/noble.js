...
const bn_js_1 = require("ethereum-cryptography/bn.js");
...
const ONE_BUFFER = (0, index_js_1.concatBytes)(new Uint8Array(31), (0, index_js_1.hexToBytes)('0x01'));
/**
...
function toG1Point(input) {
    if ((0, index_js_1.equalsBytes)(input, G1_INFINITY_POINT_BYTES)) {
        return bn_js_1.bn254.G1.ProjectivePoint.ZERO;
    }
    const x = (0, index_js_1.bytesToBigInt)(input.subarray(0, G1_ELEMENT_BYTE_LENGTH));
    const y = (0, index_js_1.bytesToBigInt)(input.subarray(G1_ELEMENT_BYTE_LENGTH, G1_POINT_BYTE_LENGTH));
    const G1 = bn_js_1.bn254.G1.ProjectivePoint.fromAffine({
        x,
        y,
    });
    G1.assertValidity();
...
function fromG1Point(input) {
    const xBytes = (0, index_js_1.setLengthLeft)((0, index_js_1.bigIntToBytes)(input.x), G1_ELEMENT_BYTE_LENGTH);
    const yBytes = (0, index_js_1.setLengthLeft)((0, index_js_1.bigIntToBytes)(input.y), G1_ELEMENT_BYTE_LENGTH);
...
function toFrPoint(input) {
    const Fr = bn_js_1.bn254.fields.Fr.fromBytes(input);
    if (Fr >= bn_js_1.bn254.fields.Fr.ORDER) {
        return Fr % bn_js_1.bn254.fields.Fr.ORDER;
    }
    return Fr;
}
/**
...
function toG2Point(input) {
    if ((0, index_js_1.equalsBytes)(input, G2_INFINITY_POINT_BYTES)) {
        return bn_js_1.bn254.G2.ProjectivePoint.ZERO;
    }
    ...
    for (const p of [p_x_1, p_x_2, p_y_1, p_y_2]) {
        const pB = (0, index_js_1.bytesToBigInt)(p);
        if (bn_js_1.bn254.fields.Fp.create(pB) !== pB) {
            throw new exceptions_js_1.EvmError(exceptions_js_1.ERROR.BN254_FP_NOT_IN_FIELD);
        }
    }
    const Fp2X = toFp2Point(p_x_1, p_x_2);
    const Fp2Y = toFp2Point(p_y_1, p_y_2);
    const pG2 = bn_js_1.bn254.G2.ProjectivePoint.fromAffine({
        x: Fp2X,
        y: Fp2Y,
    });
    pG2.assertValidity();
...
function toFp2Point(fpXCoordinate, fpYCoordinate) {
    if ((0, index_js_1.bytesToBigInt)(fpXCoordinate) >= bn_js_1.bn254.fields.Fp2.ORDER) {
        throw new exceptions_js_1.EvmError(exceptions_js_1.ERROR.BN254_FP_NOT_IN_FIELD);
    }
    if ((0, index_js_1.bytesToBigInt)(fpYCoordinate) >= bn_js_1.bn254.fields.Fp2.ORDER) {
        throw new exceptions_js_1.EvmError(exceptions_js_1.ERROR.BN254_FP_NOT_IN_FIELD);
    }
    const fpBytes = (0, index_js_1.concatBytes)(fpXCoordinate, fpYCoordinate);
    const FP = bn_js_1.bn254.fields.Fp2.fromBytes(fpBytes);
    return FP;
}
/**
...
class NobleBN254 {
    ...
    pairing(input) {
        ...
        for (let k = 0; k < input.length / pairLength; k++) {
            ...
            if (G1 === bn_js_1.bn254.G1.ProjectivePoint.ZERO || G2 === bn_js_1.bn254.G2.ProjectivePoint.ZERO) {
                continue;
            }
            pairs.push({ g1: G1, g2: G2 });
        }
        const res = bn_js_1.bn254.pairingBatch(pairs);
        if (bn_js_1.bn254.fields.Fp12.eql(res, bn_js_1.bn254.fields.Fp12.ONE) === true) {
            return ONE_BUFFER;
        }
        else {
            return ZERO_BUFFER;

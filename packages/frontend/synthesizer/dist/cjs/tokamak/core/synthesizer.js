...
exports.Synthesizer = exports.synthesizerEnvInf = exports.prepareEXTCodePt = exports.synthesizerBlkInf = exports.synthesizerArith = void 0;
const index_js_1 = require("@ethereumjs/util/index.js");
const keccak_js_1 = require("ethereum-cryptography/keccak.js");
const util_js_1 = require("../../eof/util.js");
const util_js_2 = require("../../opcodes/util.js");
const index_js_2 = require("../constant/index.js");
const index_js_3 = require("../resources/index.js");
const index_js_4 = require("../operations/index.js");
const index_js_5 = require("../pointers/index.js");
const utils_js_1 = require("../utils/utils.js");
const index_js_6 = require("../validation/index.js");
const synthesizerArith = (op, ins, out, runState) => {
...
exports.synthesizerArith = synthesizerArith;
const synthesizerBlkInf = (op, runState, target) => {
    let dataPt;
    switch (op) {
        case 'BLOCKHASH':
        case 'BLOBHASH':
...
exports.synthesizerBlkInf = synthesizerBlkInf;
async function prepareEXTCodePt(runState, target, _offset, _size) {
    const address = (0, util_js_2.createAddressFromStackBigInt)(target);
...
exports.prepareEXTCodePt = prepareEXTCodePt;
async function synthesizerEnvInf(op, runState, target, offset) {
    ...
    switch (op) {
        case 'CALLDATALOAD': {
            ...
            if (calldataMemoryPts.length > 0) {
                ...
                const calldataMemoryPt = (0, index_js_5.simulateMemoryPt)(calldataMemoryPts);
        ...
        case 'EXTCODEHASH': {
            ...
            if (codePt.value === index_js_1.BIGINT_0) {
...
exports.synthesizerEnvInf = synthesizerEnvInf;
/**
...
class Synthesizer {
    constructor() {
        this.adjustMemoryPts = (dataPts, memoryPts, srcOffset, dstOffset, viewLength) => {
        ...
        if (index_js_3.subcircuits[1].name === index_js_2.LOAD_PLACEMENT.name &&
            index_js_3.subcircuits[index_js_3.subcircuits.length - 2].name === index_js_2.RETURN_PLACEMENT.name) {
            this.placements.set(index_js_2.LOAD_PLACEMENT_INDEX, index_js_2.LOAD_PLACEMENT);
            this.placements.set(index_js_2.RETURN_PLACEMENT_INDEX, index_js_2.RETURN_PLACEMENT);
        }
        else {
            throw new Error(`Synthesizer: Placement: Initialization faield.`);
        }
        if (index_js_3.subcircuits[index_js_3.subcircuits.length - 1].name === index_js_2.KECCAK_IN_PLACEMENT.name) {
            this.placements.set(index_js_2.KECCAK_IN_PLACEMENT_INDEX, index_js_2.KECCAK_IN_PLACEMENT);
        }
        else {
            throw new Error(`Synthesizer: Placement: Initialization faield.`);
        }
        if (index_js_3.subcircuits[0].name === index_js_2.KECCAK_OUT_PLACEMENT.name) {
            this.placements.set(index_js_2.KECCAK_OUT_PLACEMENT_INDEX, index_js_2.KECCAK_OUT_PLACEMENT);
        }
        else {
            throw new Error(`Synthesizer: Placement: Initialization faield.`);
        }
        ...
        this.placementIndex = index_js_2.INITIAL_PLACEMENT_INDEX;
        this.subcircuitNames = index_js_3.subcircuits.map((circuit) => circuit.name);
        this.subcircuitInfoByName = new Map();
        for (const subcircuit of index_js_3.subcircuits) {
            const entryObject = {
                id: subcircuit.id,
    ...
    _addWireToLoadPlacement(pointerIn) {
        ...
        if (this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).inPts.length !==
            this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).outPts.length) {
            throw new Error(`Mismatches in the Load wires`);
        }
        const outWireIndex = this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).outPts.length;
        // Create output data point
        const outPtRaw = {
            source: 0,
            wireIndex: outWireIndex,
            value: pointerIn.value,
            sourceSize: index_js_2.DEFAULT_SOURCE_SIZE,
        };
        const pointerOut = index_js_5.DataPointFactory.create(outPtRaw);
        // Add input-output pair to the LOAD subcircuit
        this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).inPts.push(pointerIn);
        this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).outPts.push(pointerOut);
        return this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).outPts[outWireIndex];
    }
    /**
    ...
    loadPUSH(codeAddress, programCounter, value, size) {
        ...
        const pointerIn = index_js_5.DataPointFactory.create(inPtRaw);
        return this._addWireToLoadPlacement(pointerIn);
    }
    loadAuxin(value) {
        if (this.auxin.has(value)) {
            return this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).outPts[this.auxin.get(value)];
        }
        const inPtRaw = {
            source: 'auxin',
            value,
    ...
    loadEnvInf(codeAddress, type, value, _offset, size) {
        ...
        // }
        const sourceSize = size ?? index_js_2.DEFAULT_SOURCE_SIZE;
        const inPtRaw = {
            ...whereItFrom,
            value,
            sourceSize,
    ...
    loadStorage(codeAddress, key, value) {
        ...
        if (this.storagePt.has(keyString)) {
            ...
            const inPt = index_js_5.DataPointFactory.create(inPtRaw);
    ...
    storeStorage(codeAddress, key, inPt) {
        const keyString = JSON.stringify({ address: codeAddress, key: Number(key) });
        this.storagePt.set(keyString, inPt);
        const outWireIndex = this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).outPts.length;
        // Create output data point
        const outPtRaw = {
        ...
        const outPt = index_js_5.DataPointFactory.create(outPtRaw);
        // Add input-output pair to the ReturnBuffer
        this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).inPts.push(inPt);
        this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).outPts.push(outPt);
    }
    storeLog(valPt, topicPts) {
        this.logPt.push({ valPt, topicPts });
        let outWireIndex = this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).outPts.length;
        const inWireIndex = this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).inPts.length;
        // Create output data point
        const outPtRaw = {
            dest: 'LOG',
        ...
        const valOutPt = index_js_5.DataPointFactory.create(outPtRaw);
        // Add input-output pair to the ReturnBuffer
        this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).inPts.push(valPt);
        this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).outPts.push(valOutPt);
        ...
        for (const topicPt of topicPts) {
            ...
            const topicOutPt = index_js_5.DataPointFactory.create(outPtRaw);
            this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).inPts.push(topicPt);
            this.placements.get(index_js_2.RETURN_PLACEMENT_INDEX).outPts.push(topicOutPt);
        }
    }
    loadBlkInf(blkNumber, type, value) {
        ...
        if (this.blkInf.has(key)) {
            return this.placements.get(index_js_2.LOAD_PLACEMENT_INDEX).outPts[this.blkInf.get(key).wireIndex];
        }
        const inPtRaw = {
            ...whereItFrom,
            value,
    ...
    loadKeccak(inPts, outValue, length) {
        ...
        let value = index_js_1.BIGINT_0;
        for (let i = 0; i < nChunks; i++) {
            value += inPts[i].value << BigInt((nChunks - i - 1) * 32 * 8);
        }
        const valueInBytes = (0, index_js_1.bigIntToBytes)(value);
        const data = (0, index_js_1.setLengthLeft)(valueInBytes, Number(length) ?? valueInBytes.length);
        const _outValue = BigInt((0, index_js_1.bytesToHex)((0, keccak_js_1.keccak256)(data)));
        if (_outValue !== outValue) {
            throw new Error(`Synthesizer: loadKeccak: The Keccak hash may be customized`);
        }
        const inWireIndex = this.placements.get(index_js_2.KECCAK_IN_PLACEMENT_INDEX).inPts.length;
        const pairedInputWireIndices = Array.from({ length: nChunks }, (_, i) => inWireIndex + i);
        const outWireIndex = this.placements.get(index_js_2.KECCAK_OUT_PLACEMENT_INDEX).outPts.length;
        // Create output data point
        const outPtRaw = {
            source: index_js_2.KECCAK_OUT_PLACEMENT_INDEX,
            wireIndex: outWireIndex,
        ...
        for (let i = 0; i < nChunks; i++) {
            this.placements.get(index_js_2.KECCAK_IN_PLACEMENT_INDEX).inPts[pairedInputWireIndices[i]] = inPts[i];
            this.placements.get(index_js_2.KECCAK_IN_PLACEMENT_INDEX).outPts[pairedInputWireIndices[i]] = inPts[i];
        }
        this.placements.get(index_js_2.KECCAK_OUT_PLACEMENT_INDEX).inPts.push(outPt);
        this.placements.get(index_js_2.KECCAK_OUT_PLACEMENT_INDEX).outPts.push(outPt);
        return this.placements.get(index_js_2.KECCAK_OUT_PLACEMENT_INDEX).outPts[outWireIndex];
    }
    /**
    ...
    placeMSTORE(dataPt, truncSize) {
        ...
        if (truncSize < dataPt.sourceSize) {
            ...
            if (dataPt.value !== outValue) {
                ...
                const outPts = [index_js_5.DataPointFactory.create(rawOutPt)];
    ...
    placeEXP(inPts) {
        index_js_6.SynthesizerValidator.validateSubcircuitName('subEXP', this.subcircuitNames);
        // a^b
        const aPt = inPts[0];
        const bPt = inPts[1];
        ...
        const chPts = [];
        const ahPts = [];
        chPts.push(this.loadAuxin(index_js_1.BIGINT_1));
        ahPts.push(aPt);
        for (let i = 1; i <= k; i++) {
    ...
    validateOperation(name, inPts) {
        ...
        index_js_6.SynthesizerValidator.validateInputCount(name, inPts.length, requiredInputs);
        index_js_6.SynthesizerValidator.validateInputs(inPts);
    }
    executeOperation(name, values) {
        const operation = index_js_4.OPERATION_MAPPING[name];
        return operation(...values);
    }
    createOutputPoint(value, _wireIndex) {
        const wireIndex = _wireIndex ?? 0;
        return index_js_5.DataPointFactory.create({
            source: this.placementIndex,
            wireIndex,
            value,
            sourceSize: index_js_2.DEFAULT_SOURCE_SIZE,
    ...
    handleBinaryOp(name, inPts) {
        try {
            ...
            if (error instanceof index_js_6.InvalidInputCountError) {
                /*eslint-disable*/
                console.error(`Invalid input count for ${name}: ${error.message}`);
            }
            if (error instanceof index_js_6.SynthesizerError) {
                /*eslint-disable*/
    ...
    placeArith(name, inPts) {
        index_js_6.SynthesizerValidator.validateSubcircuitName(name, this.subcircuitNames);
        index_js_6.SynthesizerValidator.validateImplementedOpcode(name);
    ...
    _place(name, inPts, outPts) {
        ...
        (0, utils_js_1.addPlacement)(this.placements, {
            name,
            inPts,
            outPts,
        });

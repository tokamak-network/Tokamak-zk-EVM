...
const placement_js_1 = require("../constant/placement.js");
// @ts-ignore
const witness_calculator_js_1 = require("../utils/witness_calculator.js");
async function finalize(placements, validate) {
...
const halveWordSizeOfWires = (newDataPts, prevDataPt, index) => {
    ...
    newDataPts[indHigh].valueHex = (0, index_js_1.bytesToHex)((0, index_js_1.setLengthLeft)((0, index_js_1.bigIntToBytes)(newDataPts[indHigh].value), 16));
    newDataPts[indLow].valueHex = (0, index_js_1.bytesToHex)((0, index_js_1.setLengthLeft)((0, index_js_1.bigIntToBytes)(newDataPts[indLow].value), 16));
};
const removeUnusedLoadWires = (placements) => {
    const outLoadPlacement = { ...placements.get(placement_js_1.LOAD_PLACEMENT_INDEX) };
    ...
    for (let ind = 0; ind < outLoadPlacement.outPts.length; ind++) {
        ...
        for (const key of placements.keys()) {
            if (key !== placement_js_1.LOAD_PLACEMENT_INDEX) {
                const placement = placements.get(key);
                for (const [_ind, _inPt] of placement.inPts.entries()) {
                    if (_inPt.source === placement_js_1.LOAD_PLACEMENT_INDEX &&
...
function refactoryPlacement(placements) {
    ...
    for (const subcircuitInfo of index_js_2.subcircuits) {
    ...
    for (const key of placements.keys()) {
        const placement = key === placement_js_1.LOAD_PLACEMENT_INDEX ? dietLoadPlacment : placements.get(key);
        const newInPts = [];
        const newOutPts = [];
...
async function outputPlacementInstance(placements) {
    ...
    for (let i = placement_js_1.LOAD_PLACEMENT_INDEX; i <= placement_js_1.KECCAK_OUT_PLACEMENT_INDEX; i++) {
        let ins = result[i].inValues;
        let outs = result[i].outValues;
        const expectedInsLen = index_js_2.subcircuits[result[i].subcircuitId].In_idx[1];
        const expectedOutsLen = index_js_2.subcircuits[result[i].subcircuitId].Out_idx[1];
...
class Permutation {
    constructor(placements, instances) {
        this.l = index_js_2.globalWireInfo.l;
        this.l_D = index_js_2.globalWireInfo.l_D;
        ...
        this.flattenMapInverse = instances === undefined ? undefined : index_js_2.globalWireInfo.wireList;
        this.subcircuitInfoByName = new Map();
        for (const subcircuit of index_js_2.subcircuits) {
            const entryObject = {
                id: subcircuit.id,
...
const testInstances = async (instances) => {
    ...
    for (const [placementInd, instance] of instances.entries()) {
        ...
        if (reuseBuffer.has(id)) {
            ...
            const targetWasmPath = path_1.default.resolve(app_root_path_1.default.path, dir, `subcircuit${id}.wasm`);
            try {
                buffer = (0, fs_2.readFileSync)(targetWasmPath);
            }
            catch (err) {
        ...
        const witnessCalculator = await (0, witness_calculator_js_1.builder)(buffer);

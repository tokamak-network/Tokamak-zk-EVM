...
exports.verifyCode = exports.ContainerSectionType = void 0;
const errors_js_1 = require("./errors.js");
const stackDelta_js_1 = require("./stackDelta.js");
/**
...
})(ContainerSectionType = exports.ContainerSectionType || (exports.ContainerSectionType = {}));
/**
 ...
 */
function verifyCode(container, evm, mode = ContainerSectionType.RuntimeCode) {
...
function validateOpcodes(container, evm, mode = ContainerSectionType.RuntimeCode) {
    ...
    function addJump(location) {
        if (intermediateBytes.has(location)) {
            ...
            (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidRJUMP);
        }
    ...
    for (const code of container.body.codeSections) {
        ...
        while (ptr < code.length) {
            ...
            if (!reachableOpcodes.has(ptr)) {
                (0, errors_js_1.validationError)(errors_js_1.EOFError.UnreachableCode);
            }
            ...
            const opcodeInputs = stackDelta_js_1.stackDelta[opcode].inputs;
            const opcodeOutputs = stackDelta_js_1.stackDelta[opcode].outputs;
            if (minStackCurrent - opcodeInputs < 0) {
                (0, errors_js_1.validationError)(errors_js_1.EOFError.StackUnderflow);
            }
            ...
            if (maxStackNext > 1023) {
                ...
                (0, errors_js_1.validationError)(errors_js_1.EOFError.StackOverflow);
            }
            if (returningFunction && opcode === 0xe4) {
                (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidReturningSection);
            }
            lastOpcode = opcode;
            if (!opcodeNumbers.has(opcode)) {
                (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidOpcode);
            }
            if (opcode === 0xe0 || opcode === 0xe1) {
                ...
                if (tableSize === undefined) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.OpcodeIntermediatesOOB);
                }
                else if (tableSize === 0) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.RJUMPVTableSize0);
                }
                ...
                if (target >= container.header.codeSizes.length) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidCallTarget);
                }
                if (opcode === 0xe3) {
                    // CALLF
                    ...
                    if (targetOutputs === 0x80) {
                        ...
                        (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidCALLFReturning);
                    }
                    ...
                    if (targetOutputs > currentOutputs && !targetNonReturning) {
                        ...
                        (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidJUMPF);
                    }
                    ...
                    if (targetNonReturning) {
                        ...
                        if (!(minStackCurrent === maxStackCurrent && maxStackCurrent === expectedStack)) {
                            (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidStackHeight);
                        }
                    }
                    if (maxStackCurrent + container.body.typeSections[target].maxStackHeight - targetInputs >
                        1024) {
                ...
                if (target >= container.header.containerSizes.length) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidEOFCreateTarget);
                }
                if (containerTypeMap.has(target)) {
                    if (containerTypeMap.get(target) !== ContainerSectionType.InitCode) {
                        (0, errors_js_1.validationError)(errors_js_1.EOFError.ContainerDoubleType);
                    }
                }
                ...
                if (mode !== ContainerSectionType.InitCode) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.ContainerTypeError);
                }
                const target = code[ptr + 1];
                if (target >= container.header.containerSizes.length) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidRETURNContractTarget);
                }
                if (containerTypeMap.has(target)) {
                ...
                if (container.header.dataSize < endOfSlice) {
                    (0, errors_js_1.validationError)(errors_js_1.EOFError.DataLoadNOutOfBounds);
                }
            }
            ...
            const intermediates = stackDelta_js_1.stackDelta[opcode].intermediates;
            if (intermediates > 0) {
            ...
            if (stackDelta_js_1.stackDelta[opcode].terminating === undefined) {
            ...
            for (const successor of successorSet) {
                if (successor < ptr) {
                    ...
                    if (stackHeightMin[successor] !== minStackNext ||
                        ...
                        (0, errors_js_1.validationError)(errors_js_1.EOFError.UnstableStack);
                    }
                }
        ...
        if (!terminatingOpcodes.has(lastOpcode)) {
            (0, errors_js_1.validationError)(errors_js_1.EOFError.InvalidTerminator);
        }
        if (container.body.typeSections[codeSection].maxStackHeight !== maxStackHeight) {
            (0, errors_js_1.validationError)(errors_js_1.EOFError.MaxStackHeightViolation);
        }
        if (maxStackHeight > 1023) {
            // TODO verify if 1023 or 1024 is the right constant
            (0, errors_js_1.validationError)(errors_js_1.EOFError.MaxStackHeightLimit);
        }
    }
    ...
    if (sectionAccumulator.size !== container.header.codeSizes.length) {
        (0, errors_js_1.validationError)(errors_js_1.EOFError.UnreachableCodeSections);
    }
    if (containerTypeMap.size !== container.header.containerSizes.length) {
        (0, errors_js_1.validationError)(errors_js_1.EOFError.UnreachableContainerSections);
    }

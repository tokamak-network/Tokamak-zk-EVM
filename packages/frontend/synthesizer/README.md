...
### Version
- In the current version, the circuit generated by the Synthesizer performs verification on the relationship between the calldata and state information input to the transaction and the output of the transaction.
- In the next version, the Synthesizer will be updated so that the generated circuit includes verification on the integrity of the state information.

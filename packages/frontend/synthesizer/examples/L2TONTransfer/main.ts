...
const senderL2PrvKey = jubjub.utils.randomPrivateKey(
  setLengthLeft(utf8ToBytes(SENDER_L2_SEED), 32),
);
...
const TOKEN_RECEPIENT_PUB_KEY = jubjub.keygen(
  setLengthLeft(utf8ToBytes('Recepient'), 32),
).publicKey;
...
const simulationOpts: SynthesizerSimulationOpts = {
  ...
  contractAddress:
    '0x2be5e8c109e2197D077D13A82dAead6a9b3433C5' as `0x${string}`,
  userStorageSlots: [0],
  ...
  publicKeyListL2: [
    ...
    jubjub.keygen(
      setLengthLeft(
        utf8ToBytes('0x01E371b2aD92aDf90254df20EB73F68015E9A000'),
        32,
      ),
    ).publicKey,
    jubjub.keygen(
      setLengthLeft(
        utf8ToBytes('0xbD224229Bf9465ea4318D45a8ea102627d6c27c7'),
        32,
      ),
    ).publicKey,
    jubjub.keygen(
      setLengthLeft(
        utf8ToBytes('0x6FD430995A19a57886d94f8B5AF2349b8F40e887'),
        32,
      ),
    ).publicKey,
    jubjub.keygen(
      setLengthLeft(
        utf8ToBytes('0x0CE8f6C9D4aD12e56E54018313761487d2D1fee9'),
        32,
      ),
    ).publicKey,
    jubjub.keygen(
      setLengthLeft(
        utf8ToBytes('0x60be9978F805Dd4619F94a449a4a798155a05A56'),
        32,
      ),
    ).publicKey,
  ],
...
const main = async () => {
  const synthesizerOpts =
    await createSynthesizerOptsForSimulationFromRPC(simulationOpts);

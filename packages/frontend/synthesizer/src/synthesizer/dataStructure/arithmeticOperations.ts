...
export class ArithmeticOperations {
  ...
  static verifyMerkleProof(inVals: bigint[]): bigint[] {
    if (inVals.length !== 3 + POSEIDON_INPUTS) {
      throw new Error(
        `VerifyMerkleProof expected exactly ${3 + POSEIDON_INPUTS} input values, but got ${inVals.length} values`,
      );
    }
    ...
    const children = [...siblings.slice(0, childIndex), child, ...siblings.slice(childIndex)];

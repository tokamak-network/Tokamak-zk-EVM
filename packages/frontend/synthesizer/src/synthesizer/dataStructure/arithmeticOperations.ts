...
import {
  ARITH_EXP_BATCH_SIZE,
  JUBJUB_EXP_BATCH_SIZE,
  POSEIDON_INPUTS,
...
export class ArithmeticOperations {
  ...
  private static _bls12381Arith(): { mod: Function; add: Function; sub: Function; mul: Function } {
    const mod = (x: bigint) =>
      ((x % ArithmeticOperations.BLS12381MODULUS) + ArithmeticOperations.BLS12381MODULUS) %
      ArithmeticOperations.BLS12381MODULUS;
  ...
  private static _jubjubAdd(in1: bigint[], in2: bigint[]): bigint[] {
    ...
    const inv = (a: bigint): bigint => {
      let t = 0n,
        newT = 1n;
      let r = ArithmeticOperations.BLS12381MODULUS,
        newR = mod(a);
      while (newR !== 0n) {
        const q = r / newR;
  ...
  static verifyMerkleProof(inVals: bigint[]): bigint[] {
    ...
    const children = [...siblings.slice(0, childHomeIndex), child, ...siblings.slice(childHomeIndex)];

    if (parent !== poseidon_raw(children) || parentIndex !== Math.floor(childIndex / POSEIDON_INPUTS)) {
      throw new Error('verifyMerkleProof failed');
    }
    return [];
  }
  ...
  static verifyMerkleProof2x(inVals: bigint[]): bigint[] {
    ...
    const childHomeIndex = childIndex % POSEIDON_INPUTS;
    const firstChildren = [...siblings1.slice(0, childHomeIndex), child, ...siblings1.slice(childHomeIndex)];

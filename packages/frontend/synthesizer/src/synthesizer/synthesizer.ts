...
export class Synthesizer implements SynthesizerInterface {
      ...
      (async () => {
        try {
          ...
          if (_runState === undefined) {
            // runState is undefined when transaction is completed
          ...
          const currentInterpreterStep: InterpreterStep = {
            ...
            eofSection: _interpreter._env.eof?.container.header.getSectionFromProgramCounter(_runState.programCounter),
            immediate: undefined,
            error,
            eofFunctionDepth:
              _interpreter._env.eof !== undefined
                ? _interpreter._env.eof?.eofRunState.returnStack.length + 1
                : undefined,
          };
  ...
  private async _finalizeStorage(): Promise<void> {
    const computeParentsNodePts = (
      childIndexPt: DataPt,
      childPt: DataPt,
      siblingPts: DataPt[],
    ): { parentIndexPt: DataPt; parentPt: DataPt } => {
      if (siblingPts.length !== POSEIDON_INPUTS - 1) {
        throw new Error(
          `Siblings of each level for a Merkle proof should be ${POSEIDON_INPUTS - 1}, but got ${siblingPts.length}.`,
        );
      }
      const childIndex = Number(childIndexPt.value);
      const parentIndex = Math.floor(childIndex / POSEIDON_INPUTS);

      const childrenPts = [...siblingPts.slice(0, childIndex), childPt, ...siblingPts.slice(childIndex)];
    ...
    const placeMerkleProofVerification = (
      indexPt: DataPt,
      leafPt: DataPt,
      siblings: bigint[][],
      rootPt: DataPt,
      ...
      for (var level = 0; level < MT_DEPTH; level++) {
        ...
        const siblingPts: DataPt[] = thisSiblings.map(value =>
          this.addReservedVariableToBufferIn('MERKLE_PROOF', value, true),
        );
    ...
    const finalMTRootPt = this.addReservedVariableToBufferIn(
      'RES_MERKLE_ROOT',
    ...
    for (const [key, accessList] of this.state.cachedStorage.entries()) {
      ...
      placeMerkleProofVerification(indexPt, childPt, merkleProof.siblings, finalMTRootPt);
    }

    // Write cached storage changes back to TokamakL2StateManager
    // This ensures that exportState() will see the updated storage values
    console.log(`\n🔧 [_finalizeStorage] Writing storage changes to StateManager`);
    console.log(`   Total cached entries: ${this.state.cachedStorage.size}`);

    const contractAddress = new Address(toBytes(this.cachedOpts.stateManager.cachedOpts!.contractAddress));
    let writeCount = 0;

    for (const [key, accessList] of this.state.cachedStorage.entries()) {
      const keyHex = bigIntToHex(key).slice(0, 20);
      console.log(`\n  📦 [Key ${keyHex}...] Access list: ${accessList.length} entries`);
      ...
      // Log all accesses for debugging
      accessList.forEach((access, idx) => {
        console.log(`     [${idx}] ${access.access} - value: ${access.valuePt.value}`);
      });

      // Find the last write operation for this key
      let lastWriteIndex = -1;
      for (let i = accessList.length - 1; i >= 0; i--) {
        if (accessList[i].access === 'Write') {
          lastWriteIndex = i;
      ...
      // If there was a write, persist it to StateManager
      if (lastWriteIndex >= 0) {
        writeCount++;
        const cache = accessList[lastWriteIndex];
        const keyBytes = setLengthLeft(bigIntToBytes(key), 32);
        const valueBytes = setLengthLeft(bigIntToBytes(cache.valuePt.value), 32);

        // Debug: Log storage updates
        const mtIndex = this.cachedOpts.stateManager.getMTIndex(key);
        console.log(`\n💾 [SSTORE] Writing to StateManager:`);
        console.log(`  Key:      ${bigIntToHex(key)} (${key})`);
        console.log(`  Value:    ${bigIntToHex(cache.valuePt.value)} (${cache.valuePt.value})`);
        console.log(`  MT Index: ${mtIndex >= 0 ? `✅ ${mtIndex}` : '❌ NOT REGISTERED'}`);

        await this.cachedOpts.stateManager.putStorage(contractAddress, keyBytes, valueBytes);
      } else {
        console.log(`     ⏭️  No write found`);
      }
    }

    console.log(`\n✅ [_finalizeStorage] Completed: ${writeCount} writes persisted`);
  ...
  public async synthesizeTX(): Promise<RunTxResult> {
    // Reset transaction-local state (memory and stack are not persistent across transactions)
    // NOTE: cachedStorage should NOT be cleared here because _finalizeStorage() needs it
    this._state.memoryPt = new MemoryPt();
    this._state.stackPt = new StackPt();
    console.log(`\n🔄 [synthesizeTX] Transaction state reset complete (memory & stack)`);
    ...
    // Debug: Print transaction sender information
    const txSender = this.cachedOpts.signedTransaction.getSenderAddress();
    console.log(`\n🔐 [Transaction Sender Information]`);
    console.log(`  Sender Address (from tx): ${txSender.toString()}`);
    console.log(`  Contract Address (to):    ${this.cachedOpts.signedTransaction.to?.toString()}`);
  ...
  private _applySynthesizerHandler = async (
    prevInterpreterStep: InterpreterStep,
    currentInterpreterStep: InterpreterStep,
  ): Promise<void> => {
    ...
    this._preTasksForCalls(currentInterpreterStep.opcode.name as SynthesizerSupportedOpcodes, currentInterpreterStep);
  };
  ...
  /**
   * Get the TokamakL2StateManager instance used by this synthesizer
   * Used for state export/import in state channels
   *
   * @returns TokamakL2StateManager instance
   * @throws Error if TokamakL2StateManager is not available
   */
  public getTokamakStateManager(): TokamakL2StateManager {
    const stateManager = this.cachedOpts.stateManager;
    if (!stateManager || !(stateManager instanceof TokamakL2StateManager)) {
      throw new Error('TokamakL2StateManager not available in this synthesizer instance');
    }
    return stateManager;
  }
}

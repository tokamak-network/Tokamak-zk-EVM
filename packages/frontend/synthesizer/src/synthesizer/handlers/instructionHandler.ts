import {
  ...
  MemoryPts,
  synthesizerOpcodeByName,
  synthesizerOpcodeList,
  SynthesizerOpts,
...
export class InstructionHandler {
  ...
  private _createSynthesizerHandlers(): void {
    ...
    const __createArithHandler = (opName: SynthesizerSupportedArithOpcodes): void => {
      ...
      this.synthesizerHandlers.set(op, (prevStepResult, afterStepResult) => {
    ...
    const __createSysFlowHandlers = (opName: SynthesizerSupportedSysFlowOpcodes): void => {
      const op: number = synthesizerOpcodeByName[opName];
      this.synthesizerHandlers.set(op, async (prevStepResult, afterStepResult) => {
        const out: bigint | null = afterStepResult.stack[0] ?? null;
        const opts = this._createHandlerOps(opName, prevStepResult);
        // based on https://www.evm.codes/
        let nIns: number;
    ...
    (
      [
        ...
        'SAR',
        'KECCAK256',
      ] satisfies SynthesizerSupportedOpcodes[]
    ).forEach(__createArithHandler);
    (
      [
        'ADDRESS',
        'BALANCE',
    ...
    ).forEach(__createSysFlowHandlers);

    // PUSHs
    this.synthesizerHandlers.set(synthesizerOpcodeByName['PUSH1'], (prevStepResult, afterStepResult) => {
      ...
      this.parent.state.stackPt.push(this.parent.loadArbitraryStatic(out, DEFAULT_SOURCE_BIT_SIZE, staticInDesc));
    ...
    // DUPs
    this.synthesizerHandlers.set(synthesizerOpcodeByName['DUP1'], (prevStepResult, afterStepResult) => {
      ...
      if (this.parent.state.stackPt.peek(1)[0].value !== afterStepResult.stack[0]) {
        const expected = this.parent.state.stackPt.peek(1)[0].value;
        const actual = afterStepResult.stack[0];
        console.error(`\n🔴 DUP${stackPos} Output Mismatch:`);
        console.error(`  Expected (Synthesizer): 0x${expected.toString(16)}`);
        console.error(`  Actual (EVM):           0x${actual.toString(16)}`);
        console.error(
          `  Stack (EVM):            [${afterStepResult.stack
            .slice(0, 5)
            .map(v => '0x' + v.toString(16))
            .join(', ')}...]`,
        );
        throw new Error(
          `Synthesizer: DUP${stackPos}: Output data mismatch (expected: 0x${expected.toString(16)}, got: 0x${actual.toString(16)})`,
        );
      }
    });
    ...
    // SWAPs
    this.synthesizerHandlers.set(synthesizerOpcodeByName['SWAP1'], (prevStepResult, afterStepResult) => {
      const stackPos = prevStepResult.opcode.code - 0x8f;
      this.parent.state.stackPt.swap(stackPos);
      if (this.parent.state.stackPt.peek(1)[0].value !== afterStepResult.stack[0]) {
        const expected = this.parent.state.stackPt.peek(1)[0].value;
        const actual = afterStepResult.stack[0];
        console.error(`\n🔴 SWAP${stackPos} Output Mismatch:`);
        console.error(`  Expected (Synthesizer): 0x${expected.toString(16)}`);
        console.error(`  Actual (EVM):           0x${actual.toString(16)}`);
        console.error(
          `  Stack (EVM):            [${afterStepResult.stack
        ...
        throw new Error(
          `Synthesizer: SWAP${stackPos}: Output data mismatch (expected: 0x${expected.toString(16)}, got: 0x${actual.toString(16)})`,
        );
      }
    });
    ...
    // LOGs
    this.synthesizerHandlers.set(synthesizerOpcodeByName['LOG0'], (prevStepResult, afterStepResult) => {
  ...
  getOriginAddressPt(): DataPt {
    ...
    const sG: DataPt[] = this.parent.placeJubjubExp([...jubjubBasePt, ...signBits], jubjubPoIPt, signaturePt.value);

    const eA: DataPt[] = this.parent.placeJubjubExp([...publicKeyPt, ...challengeBits], jubjubPoIPt, poseidonOut.value);
  ...
  public async loadStorage(keyPt: DataPt, value?: bigint): Promise<DataPt> {
    ...
    console.log(`\n🔑 [SLOAD] Attempting to load storage:`);
    console.log(`  Key (decimal): ${key}`);
    console.log(`  Key (hex):     ${bigIntToHex(key)}`);
    console.log(`  MT Index:      ${MTIndex >= 0 ? `✅ ${MTIndex}` : '❌ NOT REGISTERED'}`);
    ...
    if (MTIndex >= 0) {
      ...
      if (!cached || cached.length === 0) {
        ...
        if (cachedKeyPt?.value !== keyPt.value) {
          throw new Error(
            'DataPts for a storage key are inconsistent between the cachedStorage and SSTORE input. Need to be debugged.',
          );
        }
        cached.push({
          keyPt,
      ...
      if (!cached || cached.length === 0) {
        this.parent.state.cachedStorage.set(key, [{ keyPt: null, valuePt: symbolDataPt, access: 'Write' as const }]);
      } else {
  ...
  public handleArith = (ins: bigint[], out: bigint, opts: HandlerOpts): void => {
      ...
      case 'KECCAK256':
        {
          ...
          const memOffset = ins[0];
          const dataLength = ins[1];
          const { chunkDataPts, dataRecovered } = this._chunkMemory(memOffset, dataLength);
  ...
  public handleBlkInf = (op: SynthesizerSupportedBlkInfOpcodes, out: bigint, blockNumber?: bigint): void => {
    ...
    switch (op) {
      ...
      case 'BLOCKHASH': {
        ...
        dataPt =
          blockNumberDiff <= 0n && blockNumberDiff > 256n
      ...
      default:
        throw new Error(`Synthesizer: ${op} is unimplemented.`);
    }
    stackPt.push(dataPt);
  ...
  private _getStaticInDataPt = (output: bigint, opts: HandlerOpts, targetAddress?: bigint): DataPt => {
    ...
    let targetDesc =
      targetAddress === undefined ? `` : `(target: ${createAddressFromBigInt(targetAddress).toString()})`;
    // return cachedDataPt ?? this.parent.loadArbitraryStatic(
    return this.parent.loadArbitraryStatic(value, DEFAULT_SOURCE_BIT_SIZE, staticInDesc + targetDesc);
  };
  ...
  private _popStackPtAndCheckInputConsistency = (ins: bigint[]): DataPt[] => {
    ...
    for (var i = 0; i < nIns; i++) {
      if (ins[i] !== dataPts[i].value) {
        console.error(`\n🔴 Input Data Mismatch at position ${i}:`);
        console.error(`  Expected (Synthesizer stackPt): 0x${dataPts[i].value.toString(16)}`);
        console.error(`  Actual (EVM stack):             0x${ins[i].toString(16)}`);
        console.error(`  All inputs (EVM):               [${ins.map(v => '0x' + v.toString(16)).join(', ')}]`);
        console.error(
          `  All dataPts (Synthesizer):      [${dataPts.map(dp => '0x' + dp.value.toString(16)).join(', ')}]`,
        );
        throw new Error(
          `Synthesizer: Handler: The ${i}-th input data mismatch (expected: 0x${dataPts[i].value.toString(16)}, got: 0x${ins[i].toString(16)})`,
        );
      }
    }
    return dataPts;
  ...
  public handleEnvInf(ins: bigint[], out: bigint | null, opts: HandlerOpts): void {
      ...
      case 'CALLDATACOPY':
        {
          ...
          if (dataLength !== BIGINT_0) {
            const calldataMemoryPts = this.parent.state.callMemoryPtsStack[opts.callDepth];
            const memPts: MemoryPts = this.parent.copyMemoryPts(calldataMemoryPts, dataOffset, dataLength, memOffset);
            memoryPt.writeBatch(memPts);
          }
          const _outData = memoryPt.viewMemory(Number(memOffset), Number(dataLength));
      ...
      case 'EXTCODECOPY':
        {
          ...
          checkRequiredInput(opts.memOut);
          if (dataLength !== BIGINT_0) {
            const memPts: MemoryPts = this._prepareCodeMemoryPts(opts.memOut!, addressBigInt, codeOffset, dataLength);
            memoryPt.writeBatch(memPts);
          }
          const _outData = memoryPt.viewMemory(Number(memOffset), Number(dataLength));
  ...
  public async handleSysFlow(ins: bigint[], out: bigint | null, opts: HandlerOpts): Promise<void> {
    ...
    switch (op) {
      ...
      case 'MLOAD':
        {
          const pos = ins[0];
          const dataAliasInfos = memoryPt.getDataAlias(Number(pos), 32);
          const mutDataPt =
            dataAliasInfos.length === 0
              ? this.parent.loadArbitraryStatic(0n)
      ...
      case 'MSTORE8':
        {
          ...
          const newDataPt =
            truncBitSize < originalDataPt.sourceBitSize
              ? this.parent.placeMSTORE(originalDataPt, truncBitSize)
              : originalDataPt;
      ...
      case 'MCOPY':
        {
          ...
          const _out = memoryPt.writeBatch(
            this.parent.copyMemoryPts(memoryPt.read(Number(srcOffset), Number(length)), srcOffset, length, dstOffset),
          );
          if (bytesToBigInt(_out) !== bytesToBigInt(opts.memOut!)) {
            throw new Error(`Synthesizer: ${op}: Output memory data mismatch`);
          }
      ...
      case 'STATICCALL':
        // Only post-tasks after executing an interpreter call are listed here. See "preTasksForCalls" for the pre-tasks.
        {
          ...
          if (toAddr >= 1n && toAddr <= 10n) {
            throw new Error(`Synthesizer: Precompiles are not implemented in Synthesizer.`);
          }
          const _out = this.parent.state.memoryPt.writeBatch(
            this.parent.copyMemoryPts(this.parent.state.cachedReturnMemoryPts, 0n, outLength, outOffset),
          );
          if (bytesToBigInt(_out) !== bytesToBigInt(opts.memOut!)) {
            throw new Error(`Synthesizer: ${op}: Return memory data mismatch`);
          }
          this.parent.state.stackPt.push(
    ...
    if (out === null) {
      ...
      if (stackPt.peek(1)[0].value !== out) {
        const expected = stackPt.peek(1)[0].value;
        console.error(`\n🔴 ${op} Output Mismatch:`);
        console.error(`  Expected (Synthesizer): 0x${expected.toString(16)}`);
        console.error(`  Actual (EVM):           0x${out.toString(16)}`);
        console.error(`  Difference:             ${expected !== out ? 'MISMATCH ❌' : 'Match ✅'}`);
        throw new Error(
          `Synthesizer: ${op}: Output data mismatch (expected: 0x${expected.toString(16)}, got: 0x${out.toString(16)})`,
        );
      }
    }
  }
  ...
  private _chunkMemory(offset: bigint, length: bigint): { chunkDataPts: DataPt[]; dataRecovered: bigint } {

...
import {
  ...
  synthesizerBlkInf,
  synthesizerEnvInf,
} from '../tokamak/core/handlers/instructionHandlers.ts';
import {
  copyMemoryRegion,
  simulateMemoryPt,
} from '../tokamak/pointers/index.js';
...
export const handlers: Map<number, OpHandler> = new Map([
  ...
  [
    ...
    function (runState, common) {
      ...
      const { chunkDataPts, dataRecovered } = chunkMemory(
        offset,
        length,
        runState.memoryPt,
      ...
      if (bytesToBigInt(data) !== dataRecovered) {
        throw new Error(
          `Synthesizer: KECCAK256: Data loaded to be hashed mismatch`,
        );
      }
      runState.stackPt.push(
        runState.synthesizer.loadAndStoreKeccak(chunkDataPts, r, length),
      );
      ...
      if (dataLength !== BIGINT_0) {
        ...
        if (calldataMemoryPts.length > 0) {
          ...
          while (chunkedLength > 0) {
            const chunkSize =
              chunkedLength >= DEFAULT_SOURCE_SIZE
                ? DEFAULT_SOURCE_SIZE
                : chunkedLength;

            const data = getDataSlice(
              runState.interpreter.getCallData(),
              dataOffset + accumOffsetShift,
              BigInt(chunkSize),
            );
            entryToCopy = {
  ...
  [
    ...
    function (runState) {
      ...
      // For Synthesizer //
      const [memOffsetPt, codeOffsetPt, dataLengthPt] =
      ...
      if (dataLength !== BIGINT_0) {
        ...
        while (chunkedLength > 0) {
          ...
          runState.memoryPt.write(
            Number(memOffset + accumOffsetShift),
            chunkSize,
            dataPt,
          );
  ...
  [
    ...
    async function (runState, common) {
      ...
      // For Synthesizer //
      const [addressPt, memOffsetPt, codeOffsetPt, dataLengthPt] =
        runState.stackPt.popN(4);
      if (
  ...
  [
    ...
    function (runState) {
      ...
      // For Synthesizer //
      const [memOffsetPt, returnDataOffsetPt, dataLengthPt] =
        runState.stackPt.popN(3);
      if (
  ...
  [
    ...
    function (runState) {
      ...
      const dataAliasInfos = runState.memoryPt.getDataAlias(
        offsetNum,
        loadSize,
      );
      let mutDataPt;
       ...
       */
      if (
        Number(pos) !== offsetNum ||
        runState.stack.peek(1)[0] !== mutDataPt.value
      ) {
        console.log('******ERROR******');
  ...
  [
    ...
    function (runState) {
      ...
      if (
        ...
        throw new Error(
          `MSTORE: Data to store mismatch between stackPt and stack`,
        );
      }
  ...
  [
    ...
    function (runState) {
      ...
      if (
        ...
        throw new Error(
          `MSTORE8: Stored data mismatch between memoryPt and memory`,
        );
      }
    },
  ],
  ...
  [
    ...
    async function (runState) {
      ...
      runState.synthesizer.storeStorage(
        runState.env.address.toString(),
        key,
        valPt,
      );
  ...
  [
    ...
    function (runState) {
      ...
      // For Synthesizer
      synthesizerEnvInf('PC', runState, bytesToBigInt(runState.env.address.toBytes()))
    },
  ],
  // 0x59: MSIZE
  [
  ...
  [
    ...
    async function (runState: RunState, common: Common) {
      ...
      if (toAddr >= 1n && toAddr <= 10n) {
        throw new Error(
          `Precompile is yet implemented in Synthesizer. Will be updated.`,
        );
      }
      ...
      if (
        !equalsBytes(
          _view,
          runState.memory.read(Number(outOffset), Number(outLength)),
        )
      ) {
        throw new Error(
          `Synthesizer: CALL: Output data stored in the parent context mismatch. Maybe the transaction is using precompiles (yet implemented in Synthesizer)`,
        );
      }
  ...
  [
    ...
    async function (runState: RunState, common: Common) {
      ...
      if (
        ...
        throw new Error(
          `Synthesizer: CALLCODE: Output data stored in the parent context mismatch. Maybe the transaction is using precompiles (yet implemented in Synthesizer)`,
        );
      }
      runState.stackPt.push(runState.synthesizer.loadAuxin(ret));
    },
  ...
  [
    ...
    async function (runState) {
      ...
      if (
        ...
        throw new Error(
          `Synthesizer: DELEGATECALL: Output data stored in the parent context mismatch. Maybe the transaction is using precompiles (yet implemented in Synthesizer)`,
        );
      }
      runState.stackPt.push(runState.synthesizer.loadAuxin(ret));
    },
  ...
  [
    ...
    async function (runState, _common) {
      if (runState.env.eof === undefined) {
        ...
        const dataPts = runState.memoryPt.read(
          Number(inOffset),
          Number(inLength),
        );
        const ret = await runState.interpreter.call(
  ...
  [
    ...
    async function (runState) {
      ...
      if (
        ...
        throw new Error(
          `Synthesizer: STATICCALL: Output data stored in the parent context mismatch. Maybe the transaction is using precompiles (yet implemented in Synthesizer)`,
        );
      }
      runState.stackPt.push(runState.synthesizer.loadAuxin(ret));
    },

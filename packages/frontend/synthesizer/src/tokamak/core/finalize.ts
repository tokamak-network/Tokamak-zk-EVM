...
const halveWordSizeOfWires = (
  ...
  try {
    ...
    if (prevDataPt.pairedInputWireIndices !== undefined) {
      ...
      newDataPts[Number(indHigh)].pairedInputWireIndices =
        prevDataPt.pairedInputWireIndices.flatMap(convertIndices);
      newDataPts[Number(indLow)].pairedInputWireIndices =
        prevDataPt.pairedInputWireIndices.flatMap(convertIndices);
    }

    // value가 문자열로 들어올 경우를 대비
    const value =
      typeof prevDataPt.value === 'string'
        ? BigInt(prevDataPt.value)
        : prevDataPt.value;
...
function refactoryPlacement(placements: Placements): Placements {
  ...
  for (const subcircuitInfo of subcircuitInfos) {
    subcircuitInfoByName.set(subcircuitInfo.name, {
  ...
  if (
    ...
    subcircuitInfoByName.get('bufferPrvIn')!.In_idx[1]
  ) {
    throw new Error(
      `Synthesizer: Insufficient private input buffer length. Ask the qap-compiler for longer buffers.`,
    );
  }
  if (
    outPlacements.get(PRV_OUT_PLACEMENT_INDEX)!.outPts.length >
    subcircuitInfoByName.get('bufferPrvOut')!.Out_idx[1]
  ) {
    throw new Error(
      `Synthesizer: Insufficient private output buffer length. Ask the qap-compiler for longer buffers.`,
    );
  }
  if (
    outPlacements.get(PUB_IN_PLACEMENT_INDEX)!.inPts.length >
    subcircuitInfoByName.get('bufferPubIn')!.In_idx[1]
  ) {
    throw new Error(
      `Synthesizer: Insufficient public input buffer length. Ask the qap-compiler for longer buffers.`,
    );
  }
  if (
    outPlacements.get(PUB_OUT_PLACEMENT_INDEX)!.outPts.length >
    subcircuitInfoByName.get('bufferPubOut')!.Out_idx[1]
  ) {
    throw new Error(
      `Synthesizer: Insufficient public output buffer length. Ask the qap-compiler for longer buffers.`,
    );
  }
...
export class Permutation {
  ...
  async outputPlacementVariables(
    ...
    const placementVariables: PlacementVariables = await Promise.all(
      Array.from(placements.entries()).map(async ([placementId, placement]) => {
        ...
        if (placement.subcircuitId < INITIAL_PLACEMENT_INDEX) {
          const expectedInsLen =
            subcircuitInfos[placement.subcircuitId].In_idx[1];
          if (expectedInsLen > inValues.length) {
            const filledIns = inValues.concat(
              Array(expectedInsLen - inValues.length).fill('0x00'),
            );
            inValues = filledIns;
          }
        ...
        let variables = await generateSubcircuitWitness(
        ...
        process.stdout.write(
          `Synthesizer: Instances of the ${placementId}-th placement passed the ${placement.subcircuitId}-th subcircuit.`,
        );
    ...
    console.log('');
    console.log(
      `Synthesizer: All the placement instances passed the subcircuits`,
    );

    // Extracting public instance from the placement variables
    let idxSetPubIn = new IdxSet(
      this.subcircuitInfoByName.get(
        this.placements.get(PUB_IN_PLACEMENT_INDEX)!.name,
      )!,
    );
    let idxSetPubOut = new IdxSet(
      this.subcircuitInfoByName.get(
        this.placements.get(PUB_OUT_PLACEMENT_INDEX)!.name,
      )!,
    );
    let a: string[] = [];
    ...
    for (let globalIdx = 0; globalIdx < idxSetPubIn.NInWires; globalIdx++) {
      a[globalIdx] =
        placementVariables[this.flattenMapInverse[globalIdx][0]].variables[
          this.flattenMapInverse[globalIdx][1]
        ];
    }
    for (
      let globalIdx = idxSetPubIn.NInWires;
      globalIdx < setupParams.l;
      globalIdx++
    ) {
      a[globalIdx] =
        placementVariables[this.flattenMapInverse[globalIdx][0]].variables[
    ...
    const publicInputBuffer = {
      ...
      inPts: this.placements
        .get(PUB_IN_PLACEMENT_INDEX)!
        .inPts.map(({ value, ...rest }) => rest),
      outPts: this.placements
        .get(PUB_IN_PLACEMENT_INDEX)!
        .outPts.map(({ value, ...rest }) => rest),
    };
    const publicOutputBuffer = {
      ...this.placements.get(PUB_OUT_PLACEMENT_INDEX)!,
      inPts: this.placements
        .get(PUB_OUT_PLACEMENT_INDEX)!
        .inPts.map(({ value, ...rest }) => rest),
      outPts: this.placements
        .get(PUB_OUT_PLACEMENT_INDEX)!
        .outPts.map(({ value, ...rest }) => rest),
    ...
    const filePath2 =
      _path === undefined
        ? path.resolve(appRootPath.path, 'examples/outputs/publicInstance.json')
    ...
    const files = [
      placementVariablesJson,
      publicInstanceJson,
      privateExternalInterfaceJson,
    ];
  ...
  private _correctPermutation(): {
    ...
    for (const _group of this.permGroup) {
      ...
      if (groupLength > 1) {
        const reprVal = this._retrieveDataPtFromPlacementWireId(
          JSON.parse(group[0]),
        ).value;
        for (let i = 0; i < groupLength; i++) {
          ...
          const nextVal =
            this._retrieveDataPtFromPlacementWireId(nextElement).value;
          if (reprVal !== nextVal) {
          ...
          if (
            rowIdx >= this.placements.size ||
            colIdx >= setupParams.l_D - setupParams.l
          ) {
  ...
  private _buildPermGroup = (): Map<string, boolean>[] => {
    ...
    for (const placeId of this.placements.keys()) {
      ...
      const thisSubcircuitInfo = this.subcircuitInfoByName.get(
        thisPlacement.name,
      )!;
      for (let i = 0; i < thisSubcircuitInfo.NOutWires; i++) {
        ...
        if (
          !(globalWireId >= setupParams.l && globalWireId < setupParams.l_D)
        ) {
          break;
        }
    ...
    for (const thisPlacementId of this.placements.keys()) {
      ...
      for (let i = 0; i < thisSubcircuitInfo.NInWires; i++) {
        ...
        const thisGlobalWireId =
          thisSubcircuitInfo.flattenMap![thisLocalWireId];
        if (
          !(
            thisGlobalWireId >= setupParams.l &&
            thisGlobalWireId < setupParams.l_D
          )
        ) {
          break;
        }
        ...
        if (thisInPt !== undefined && thisInPt.source !== thisPlacementId) {
          ...
          const pointedLocalWireId =
            pointedSubcircuitInfo.outWireIndex + pointedOutputId;
          const pointedGlobalWireId =
            pointedSubcircuitInfo.flattenMap![pointedLocalWireId];
          ...
          if (
            !(
              pointedGlobalWireId >= setupParams.l &&
              pointedGlobalWireId < setupParams.l_D
            )
          ) {
            throw new Error(
              `Permutation: A wire is referring to a public wire or an internal wire.`,
            );
          }
        ...
        if (!hasParent) {
          ...
          if (
            thisInPt === undefined ||
            thisInPt.source === PRV_IN_PLACEMENT_INDEX
          ) {
            isQualified = true;
          }
          if (!isQualified) {
  ...
  private _validatePermutation = (): void => {
    if (this.placementVariables.length === 0) {
      throw new Error(
        'Permutation: PlacementVariables are yet built. Run "Placement.outputPlacementVariables" first.',
      );
    }
    ...
    for (const [
      ...
      const subcircuitInfo = this.subcircuitInfoByName.get(
        this.placements.get(placementId)!.name,
      )!;

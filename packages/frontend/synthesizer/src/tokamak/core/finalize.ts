...
function refactoryPlacement(synthesizer: Synthesizer): Placements {
  ...
  const finalPlacements = _processInputWires(
    placements,
    dietLoadPlacment,
    outPlacements,
  ...
  _validateBufferSizes(finalPlacements, subcircuitInfoByName);
...
function _validateBufferSizes(
  ...
  if (
    ...
    flags[0] = false;
    console.log(
      `Error: Synthesizer: Insufficient private input buffer length. Ask the qap-compiler for a longer buffer (required length: ${outPlacements.get(PRV_IN_PLACEMENT_INDEX)!.inPts.length}).`,
    );
  }
  if (
    ...
    flags[1] = false;
    console.log(
      `Error: Synthesizer: Insufficient private output buffer length. Ask the qap-compiler for a longer buffer (required length: ${outPlacements.get(PRV_OUT_PLACEMENT_INDEX)!.outPts.length}).`,
    );
  }
  if (
    ...
    flags[2] = false;
    console.log(
      `Error: Synthesizer: Insufficient public input buffer length. Ask the qap-compiler for a longer buffer (required length: ${outPlacements.get(PUB_IN_PLACEMENT_INDEX)!.inPts.length}).`,
    );
  }
  if (
    ...
    flags[3] = false;
    console.log(
      `Error: Synthesizer: Insufficient public output buffer length. Ask the qap-compiler for a longer buffer (required length: ${outPlacements.get(PUB_OUT_PLACEMENT_INDEX)!.outPts.length}).`,
    );
  }
  ...
  async outputPlacementVariables(
    ...
    const placementVariables: PlacementVariables = await Promise.all(
      Array.from(placements.entries()).map(async ([placementId, placement]) => {
        ...
        const expectedOutsLen =
          subcircuitInfos[placement.subcircuitId].Out_idx[1];
        if (expectedOutsLen > outValues.length) {
          const filledOuts = outValues.concat(
            Array(expectedOutsLen - outValues.length).fill('0x00'),
          );
    ...
    console.log('');
    console.log(
      `Synthesizer: All ${placements.size} placement instances passed the subcircuits`,
    );
    ...
    let idxSetPrvIn = new IdxSet(
      this.subcircuitInfoByName.get(
        this.placements.get(PRV_IN_PLACEMENT_INDEX)!.name,
      )!,
    );
    let idxSetPrvOut = new IdxSet(
      this.subcircuitInfoByName.get(

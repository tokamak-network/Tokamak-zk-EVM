...
export function poseidon(msg: Uint8Array): Uint8Array {
  ...
  const fold = (arr: bigint[]): bigint[] => {
    ...
    let placeFunction = mode2x ? ArithmeticOperations.poseidonN2xCompress : ArithmeticOperations.poseidonN;
...
export function eddsaSign(prvKey: bigint, msg: Uint8Array[]): { R: EdwardsPoint; S: bigint } {
    ...
    bytesToBigInt(
      poseidon(
        concatBytes(
          DST_NONCE,
          nonceKeyBytes,
          batchBigIntTo32BytesEach(pubKey.toAffine().x, pubKey.toAffine().y),
          ...msg,
        ),
      ),
  ...
  const e = bytesToBigInt(
    poseidon(
      concatBytes(
        batchBigIntTo32BytesEach(R.toAffine().x, R.toAffine().y, pubKey.toAffine().x, pubKey.toAffine().y),
        ...msg,
      ),
    ),
  );

...
const exampleType = 'private-state-transfer';
...
const buildGroupKey = (config: PrivateStateTransferConfig) => {
...
const comparePermutations = async (configs: string[]) => {
  ...
  for (const configFile of configs) {
    ...
    const config = JSON.parse(await fs.readFile(configPath, 'utf8')) as PrivateStateTransferConfig;
    ...
    if (!existing.contents.equals(permutationContents)) {
      throw new Error(
        [
          'Permutation mismatch for private-state transfer group:',
          `  key=${groupKey}`,
...
const main = async () => {
  ...
  try {
    console.log('[private-state-transfer-main] Preparing fresh anvil configs');
    ...
    if (configs.length === 0) {
      throw new Error(`No private-state transfer config files found in ${configDir}`);
    }
    ...
    for (const configFile of configs) {
      ...
      console.log(`[private-state-transfer-main] Running ${configFile}`);
      try {
        ...
        console.error(`[private-state-transfer-main] Failed for config: ${configPath}`);
        throw error;
      }
      await copyOutputs(destination);
    }

    console.log('[private-state-transfer-main] Validating permutation.json consistency');
    await comparePermutations(configs);
    console.log('[private-state-transfer-main] Validating instance_description.json contents');

...
import {
  ...
  deriveParticipantKeys as deriveMintParticipantKeys,
  getExampleRpcUrl as getMintRpcUrl,
  loadConfig as loadMintConfig,
  toStateManagerChannelConfig as toMintStateManagerChannelConfig,
  type PrivateStateMintConfig,
} from './privateState/mintNotes/utils.ts';
import {
  buildPrivateStateRedeemCalldata,
  deriveParticipantKeys as deriveRedeemParticipantKeys,
  getExampleRpcUrl as getRedeemRpcUrl,
  loadConfig as loadRedeemConfig,
  toStateManagerChannelConfig as toRedeemStateManagerChannelConfig,
  type PrivateStateRedeemConfig,
} from './privateState/redeemNotes/utils.ts';
import {
  buildPrivateStateTransferCalldata,
  deriveParticipantKeys as deriveTransferParticipantKeys,
  getExampleRpcUrl as getTransferRpcUrl,
  loadConfig as loadTransferConfig,
  toStateManagerChannelConfig as toTransferStateManagerChannelConfig,
  type PrivateStateTransferConfig,
} from './privateState/transferNotes/utils.ts';
...
const configAdapters: Record<ConfigExampleType, ConfigAdapter<any>> = {
  ...
  'private-state-mint': {
    loadConfig: loadMintConfig,
    getRpcUrl: (config: PrivateStateMintConfig, env) => getMintRpcUrl(config.network, env),
    deriveParticipantKeys: (config: PrivateStateMintConfig) => deriveMintParticipantKeys(config.participants),
    ...
    toStateManagerChannelConfig: toMintStateManagerChannelConfig,
  ...
  'private-state-redeem': {
    loadConfig: loadRedeemConfig,
    getRpcUrl: (config: PrivateStateRedeemConfig, env) => getRedeemRpcUrl(config.network, env),
    deriveParticipantKeys: (config: PrivateStateRedeemConfig) => deriveRedeemParticipantKeys(config.participants),
    ...
    toStateManagerChannelConfig: toRedeemStateManagerChannelConfig,
  ...
  'private-state-transfer': {
    loadConfig: loadTransferConfig,
    getRpcUrl: (config: PrivateStateTransferConfig, env) => getTransferRpcUrl(config.network, env),
    deriveParticipantKeys: (config: PrivateStateTransferConfig) => deriveTransferParticipantKeys(config.participants),
    ...
    toStateManagerChannelConfig: toTransferStateManagerChannelConfig,

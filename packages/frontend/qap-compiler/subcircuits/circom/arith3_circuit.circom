...
template Arith3 () {
    ...
    */
    ...
    // inter[0] <== [
    //     b_selector[5] * sdiv_out[0],
    //     b_selector[5] * sdiv_out[1]
    // ];

    // // operator 0x06: mod
    // inter[1] <== [
    //     inter[0][0] + b_selector[6] * mod_out[0],
    //     inter[0][1] + b_selector[6] * mod_out[1]
    // ];

    // // operator 0x07: smod
    // inter[1] <== [
    //     inter[0][0] + b_selector[7] * smod_out[0],
    //     inter[0][1] + b_selector[7] * smod_out[1]
    // ];
    
    
    // operator 0x08: addmod
    signal addmod_out[2] <== AddMod()(in1, in2, in3);
    inter[0] <== [
        b_selector[7] * addmod_out[0],
        b_selector[7] * addmod_out[1]
    ];

    // operator 0x09: mulmod
    signal mulmod_out[2] <== MulMod()(in1, in2, in3);
    inter[1] <== [
        inter[0][0] + b_selector[8] * mulmod_out[0],
        inter[0][1] + b_selector[8] * mulmod_out[1]
    ];
...
component main {public [in1, in2, selector]} = Arith2();

...
use std::process;
use tokamak_groth16_verifier::{Groth16Verifier, Result};
...
fn run() -> Result<()> {
    ...
    if args.len() != 4 {
        eprintln!("Usage: {} <verification_key.json> <proof.json> <public_inputs.json>", args[0]);
        eprintln!();
        eprintln!("This program verifies a Groth16 proof for Tokamak storage Merkle proofs.");
        eprintln!("Returns 1 if the proof is valid, 0 if invalid.");
        eprintln!();
        eprintln!("Arguments:");
        eprintln!("  verification_key.json  - Path to the verification key file");
        eprintln!("  proof.json            - Path to the proof file");
        eprintln!("  public_inputs.json    - Path to the public inputs file");
        eprintln!();
        eprintln!("Example:");
        eprintln!("  {} ../trusted-setup/ceremony_output/verification_key.json \\", args[0]);

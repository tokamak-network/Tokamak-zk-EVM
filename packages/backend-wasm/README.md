...
Runtime prover and verifier APIs must consume and produce binary bundles. JSON, rkyv, native artifact conversion, fixture import, and debug export belong to tooling outside the hot prover and verifier paths.
...
- `serialization/`: backend-wasm binary bundle format, section table encoding, schema checks, and digest validation.
...
Local development and validation scripts. These scripts check fixtures, binary bundle behavior, runtime arithmetic, polynomial parity, and verifier parity.
...
Verifier runtime input is split into two binary bundles:

- `VerifierProofInput`: instance data and proof payload only.
- `VerifierSetupInput`: `sigma_verify`, verifier preprocess data, setup/domain metadata, and compatibility metadata.
